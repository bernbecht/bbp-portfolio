...
export default function Portrait() {
  ...
  return (
    <button
      type="button"
      className="portrait hidden border-r border-strong focus-visible:z-10 focus-visible:outline-offset-[-4px] layout:col-span-4 layout:block"
      aria-label="Show Bernardo’s portrait in color"
      aria-pressed={revealed}
      onClick={() => setRevealed((value) => !value)}
      onPointerMove={followPointer}
      onPointerLeave={(event) => {
        event.currentTarget.dataset.following = "false";
      }}
    ...
    >
      ...
      <span
        aria-hidden="true"
        className="absolute bottom-3 right-3 border border-inverse-border bg-inverse px-2 py-1 type-label text-on-inverse"
      >
